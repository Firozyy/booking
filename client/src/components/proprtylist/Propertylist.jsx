...
function Propertylist() {
    ...
    const { data, loading, error, } = useFetch(resources.countbytype);

    const { type } = data

    return (

        <Row className='pList mt-3'>
            <h1 className="homeTitle mb-4">Browse by property type</h1>

            {type && imgs.map((imgs, i) => (

                <Col sm={6} md={3} className="pListItems">

                    <img src={imgs} alt="" className="pListImg" />
                    <div className="plistTitle">
                        <h1>{type[i].item}</h1>
                        <h2>{type[i].count} properties</h2>
                    </div>
                </Col>
            ))
